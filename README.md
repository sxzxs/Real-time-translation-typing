...
## 使用
* 鼠标拖动`实时打字翻译-网页版(推荐).ah2`到`Autohotkey.exe`上面
* 或者修改`实时打字翻译-网页版(推荐).ah2`的文件名为`实时打字翻译-网页版(推荐).ahk`，然后修改`Auohotkey.exe`的文件名为`实时打字翻译-网页版(推荐).exe` 双击`实时打字翻译-网页版(推荐).exe`即可
